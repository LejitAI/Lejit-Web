...
const Ocr = ({ initialFile }) => {
    ...
    const handleExtractText = async (fileToProcess = file) => {
        ...
        try {
            const { data: { text } } = await axios.post('http://localhost:5000/api/vision', formData, {
    ...
    const handleFormat = async () => {
        ...
        try {
            const { data: { formattedText } } = await axios.post('http://backend.lejit.ai/api/format', { text: extractedText });
